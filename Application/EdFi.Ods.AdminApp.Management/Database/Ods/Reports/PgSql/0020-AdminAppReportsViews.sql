...
CREATE OR REPLACE VIEW adminapp.StudentDemographicsReport  
AS
...
WHERE ((ssa.ExitWithdrawDate IS NULL) OR (ssa.ExitWithdrawDate >= NOW()));
...
CREATE OR REPLACE VIEW  adminapp.LocalEducationAgenciesReport 
AS
...
CREATE OR REPLACE VIEW adminapp.DistrictTotalEnrollmentsReport 
AS
SELECT COUNT(DISTINCT s.StudentUSI) AS TotalEnrollment, sch.LocalEducationAgencyId
FROM edfi.Student AS s
...
CREATE OR REPLACE VIEW adminapp.DistrictSchoolsByTypeReport 
AS
SELECT d.ShortDescription AS Description, COUNT(*) AS Count, LocalEducationAgencyId
...
CREATE OR REPLACE VIEW adminapp.StudentsByProgramReport
AS
SELECT
spa.ProgramName, spa.EducationOrganizationId, sch.LocalEducationAgencyId
FROM edfi.Student AS s
INNER JOIN edfi.StudentSchoolAssociation AS ssa ON s.StudentUSI = ssa.StudentUSI
INNER JOIN edfi.School AS sch ON (ssa.SchoolId = sch.SchoolId) 
INNER JOIN edfi.GeneralStudentProgramAssociation AS spa ON (spa.StudentUSI = s.StudentUSI)
WHERE ((spa.EndDate > NOW()) OR (spa.EndDate IS NULL))
AND ((ssa.ExitWithdrawDate IS NULL) OR (ssa.ExitWithdrawDate >= NOW()));
...
CREATE OR REPLACE VIEW adminapp.StudentEconomicSituationReport
AS
SELECT DISTINCT s.StudentUniqueId,
...
LEFT JOIN edfi.studentschoolfoodserviceprogramassociationschoolfoodserv_85a0eb fsps ON fsps.StudentUSI = s.StudentUSI AND fsps.EducationOrganizationId = sch.SchoolId
