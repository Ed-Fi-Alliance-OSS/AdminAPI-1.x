...
{
    public class AwsTabDisplayService : BaseTabDisplayService, ITabDisplayService
    {
        ...
        {
            var instanceSettingsTabs = base.GetOdsInstanceSettingsTabDisplay(selectedTab);

            var tabsToDisable = instanceSettingsTabs.Where(x =>
                x.Tab == OdsInstanceSettingsTabEnumeration.Logging ||
                x.Tab == OdsInstanceSettingsTabEnumeration.BulkLoad);
